    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6 col-sm-12">
                    <div class="form-group">
                        <label class="required" for="product_color_id">{{ trans('cruds.product.fields.product_color') }}</label>
                        <select class="form-control select2 {{ $errors->has('product_color') ? 'is-invalid' : '' }}" name="product_color_id" id="product_color_id" required>
                            @foreach($product_colors as $id => $entry)
                                <option value="{{ $id }}" {{ (old('product_color_id') ? old('product_color_id') : $product->product_color->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-lg-4 col-md-6 col-sm-12">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('old_price') ? 'is-invalid' : '' }}" type="number" name="old_price" id="old_price" value="{{ old('old_price', $product->old_price) }}" step="0.01">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12">
                    <div class="form-group">
                        <label class="required" for="product_thumbnail_image">{{ trans('cruds.product.fields.product_thumbnail_image') }}</label>
                        <div class="needsclick dropzone {{ $errors->has('product_thumbnail_image') ? 'is-invalid' : '' }}" id="product_thumbnail_image-dropzone">
                        </div>
                        @if($errors->has('product_thumbnail_image'))
                            <div class="invalid-feedback">
                                {{ $errors->first('product_thumbnail_image') }}
                            </div>
                        @endif
                        <span class="help-block">{{ trans('cruds.product.fields.product_thumbnail_image_helper') }}</span>
                    </div>
                </div>
...
<script>
    Dropzone.options.productThumbnailImageDropzone = {
    url: '{{ route('admin.products.storeMedia') }}',
    maxFilesize: 2, // MB
    acceptedFiles: '.jpeg,.jpg,.png,.gif',
    maxFiles: 1,
    ...
    success: function (file, response) {
      $('form').find('input[name="product_thumbnail_image"]').remove()
      $('form').append('<input type="hidden" name="product_thumbnail_image" value="' + response.name + '">')
    },
    removedfile: function (file) {
      file.previewElement.remove()
      if (file.status !== 'error') {
    ...
    init: function () {
@if(isset($product) && $product->product_thumbnail_image)
      var file = {!! json_encode($product->product_thumbnail_image) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="product_thumbnail_image" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
    error: function (file, response) {

...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.shippingCharge.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.shipping-charges.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="tax_name">{{ trans('cruds.shippingCharge.fields.tax_name') }}</label>
                <input class="form-control {{ $errors->has('tax_name') ? 'is-invalid' : '' }}" type="text" name="tax_name" id="tax_name" value="{{ old('tax_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tax_rate_in') ? 'is-invalid' : '' }}" type="number" name="tax_rate_in" id="tax_rate_in" value="{{ old('tax_rate_in', '') }}" step="1" required>
